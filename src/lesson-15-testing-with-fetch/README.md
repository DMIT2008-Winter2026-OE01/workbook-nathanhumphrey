...
```
npm install --save-dev jest@29.7.0 jest-environment-jsdom@29.7.0 @testing-library/react@13.4.0 @testing-library/jest-dom@5.16.5
```
...
```
npm install msw@2.0.9 isomorphic-fetch undici@5.28.2 --save-dev
```
...
```
> NOTE: Here we use `setupServer` with one argument, if you wanted to handle multiple requests just keep on adding them as different arguments. You could also add of the `rest` requests in an array and then spread that array in the `setupServer` as 
> arguments. 
...
```
This is using a function because it needs to wait for the `useEffect` to fire on mount.
...
4. Let's write a test that checks if the quote is changed on the `Home` component after the button is clicked.
```jsx
// ... imports here ...
...
Second, we mock the server to respond with the `NEW_QUOTE` and `NEW_AUTHOR` with the function `server.use` and passing in a `rest` request. Third, we get the elements and "await" the button click. Fourth, we test to make sure that our component has been updated successfully.
