...
3. In the `reviews-mock-backend`, take a look at the `package.json` file. You'll see in `scripts` a command for `server`, whic executes `json-server --watch reviews-db.json --port 5000`. 
   - run the command `npm run server` in the terminal (of course once you've done an `npm install`). You should see something like the following. Leave this running.
      ```bash
      ...
      > json-server --watch review-db.json
        ...
        Watching...
      ```
   - Let's also take a look at the `reviews-db.json` file. When we make POST requests, this file will change; it acts as our database and our RESTful API backend all at once! (As a note, if you have any errors in your `reviews-db.json` you can always copy below as your starting point.)
      ```json
      {
        "reviews": [
          {
      ...
      ```
4. Once you have the server running, open a REST API client and let's make a couple of requests.
...
the response below should reflect the changes in the `reviews-db.json` file.
      ...
      ```
      > Note: Hopefully these steps clarify how we're going to use our mock-backend. Take note that we're going to be running in separate terminals for this, but you don't need to stress, once you run the command `npm run server` you just leave it running.
5. Let's take a look at the `index.js` page in our frontend. Make sure our frontend is running (`npm run dev` in the folder `reviews-rest-app`).
...
6. Let's refactor the card content so that it simplifies the map for us.
...
7. Let's use fetch to get all of the adaptation review data and modify the state to do so.
   ...
   - In the jsx
      ...
      ```
   - Then, let's import `useState` from `react` (in our `index.js` file) and let's make a stateful array for our reviews.
      ```js
   ...
   - Fetch the data in the load all reviews button. To do this, modify the `loadAllReviewsButton` like so.
      ...
      ```
   - we need to loop through our new statful `reviews` variable and also set that variable using `setReviews` in the `loadAllReviewsButton`. Change the `MOCK_ADAPTATION_RATING` to `reviews`
      ```js
     ...
     > - You might be thinking "why don't I fetch this on load?", and you're right! We'll be learning this when we take a look at the component lifecycle and `useEffect`.
8. Let's provide the functionality to be able to POST a new review and save it to our database.
   ...
   - Let's add a stateful variables for the text fields, so that we can keep track of the state of the `TextField`s
      ...
      ```
   - On both `TextField`s we use a callback function on the `onChange` event to execute a function that calls the `setTitle` and `setComments`. We also set the "value" to the stateful variables of `title` and `comment` respectively like follows.
      ```jsx
      <>
   ...
   - To make sure that everthing is working correctly you `console.log` these variables in your handler function
    ...
    ```
9. We also have to get the rating, so let's do the same for our `RadioGroup`.
...
10. Post the data to the server using the stateful variables by modifying the `handleSubmit` to make a "POST" request using `fetch`
    ```js
    const handleSubmit = (event) => {
      event.preventDefault()
      fetch(`http://localhost:5000/reviews`, {
...
11. Save the review to the new list.
    - modify the console 
      ```js
      const handleSubmit = (event) => {
        event.preventDefault();
