...
1. Navigate into the folder `state-events-conditionals-example/` and observe what is there already.
    - This project includes MUI
    - There is a list on the index page with the data imported from the `utils/movies.js` file, which contains highly rated movies from imdb.
    - The UI includes a form 
      - We're going to use stateful variables to change what's displayed on the page.
2. Import `useState` and create variables for the `year` and `search` variables.
    ...
    - create the variables under the function definition of `Home`:
      ...
      ```
      This gives us the ability to modify the state
    - Next, we want to update these values whenever the `TextField`s change.

3. Create functions `setSearch` and `setYear` on their respective text fields
    - for the search text field add the following lines:
      ...
      />
      ```
      > Here, we have a callback that passes in `event` as a parameter, which is fired every time this `TextField` changes using the `onChange` prop. We are setting the `value` of the text field as it's being updated.
...
4. On the `form`, we're going to listen for and handle submissions, and we're going to prevent the default action:
    - create the event handler for the form submission. We're also going to print out the stateful variables we've created.
      ...
      You can also see that the `year` and `search` variables are updated to what the `TextField`s currently contain.
    - On the `form`, we're going to register this handler to the `onSubmit` event prop like so:
      ```jsx
      <form
        style={{width: '100%'}}
...
5. We're going to set a filter on the list: 
    - create a stateful variable named `movies`
      ```jsx
      const [movies, setMovies] = useState(MOVIE_LIST);
      ```
      > Note: the original value is the `MOVIE_LIST` as that's what's passed as an argument to the function `useState`.
...
6. Create a function called `filterMovies` that will use all of the variables so that it can be filtered in the correct way:
    ...
    - we iterate over `filteredMovieList` so that the new list is a subset of only the matches to the search queries.
      - remember that `filter` returns a new array and returns a Boolean value `true` if you want to keep the result in the array and `false` if you want to filter it out.
...
9. Let's do one more validation to expand on this idea, our "year" `TextField` is a text field, in later versions we could change the type to number but that's okay for now because we will check if the stateful `year` variable is a number.
    ...
    - we're going to validate the search and display a message to the user, first we're going to have to create a stateful variable called "errorMessage" like so the following:
      ...
      ```
    - we're going to create a function named `validateSearch` that will set an error message if the year is not a number, like so:
      ```jsx
