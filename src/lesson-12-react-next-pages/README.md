...
Almost all Apps have different pages, and with Next.js we want to learn and understand the page functionality. This is going to allow us to go back and forth and navigate around our application.

# Steps

1. First, let's take a look at the `pages` directory. There's two files there now, `index.js` and `about.js`.
    > Note: You'll want to install the dependencies and run your server (`npm install` then `npm run dev`)
...
3. What we want to do in this application is add some Agencies, and give them their own page so that we can get more details on them. Let's render the agencies first.
   ...
   - Now that we have our `agenciesData` being set, notice that in our components we have `components/AgencyCard.js`, and there's a bunch of props that are being set to take a look at.
   - Let's loop through the `agenciesData` (using `map`) and create an `AgencyCard` for each agency in the empty `Box` component in `pages/index.js`.
      ```jsx
      <Box
        sx={{
          marginTop: 2,
...
4. Let's create an "agency" page in our app and navigate to it via the id.
   ...
   - in the file `pages/agency/[id].js` add the following contents:
      ...
      ```
      This is essentially just a plain old page in Next.js. You can navigate to this page by going to `http://localhost:3001/agency/4` and instead of `4` in the url you can put whatever you want.
   - Let's modify this page so that it uses the url arguments. First, you'll have to import the `useRouter` hook from `next/router` in the `pages/agency/[id].js` like below. We'll also initialize the router so that we can use the "id" in our component.
      ...
      ```
      Now you can see the id is being rendered on the page, observe the differences between the urls `http://localhost:3001/agency/200` and `http://localhost:3001/agency/4` in your browser.
5. Let's Navigate to that page from the "Home" page via the `AgencyCard` component, now that we have a page that is unique per "id" for each agency.
   ...
   - Now let's add a callback function to the buttons' `onClick` event to navigate to our new "agency" page. You'll use the `router.push` function with the path as the parameter
      ...
      ```
      Now that you have this callback go to the page when you click the "Go To Agency" button, you can see that our application navigates to each id; you can see that rendered on the page.
6. On the `pages/agency/[id].js` let's load the specific agency data using the `getAgency` method in `utils/api/agencies.js`.
   ...
   - let's load the data on on the agency page using "useEffect" but by passing the "id" to the dependency array from the `const { id } = router.query` like so.
      ...
      export default function Agency() {
          ...
          // ... rest of the component ...
      ```
   - once we have the data loaded, let's change our JSX to use a ternary operator to display the information. (Notice the `components/LoadingCircle.js` component, that is just a spinner). Our component now becomes:
      ```jsx
      export default function Agency() {
        const [agencyDetails, setAgencyDetails] = useState();
   ...
   - Let's add one more section for the spacecraft.
      ...
      ```
      Notice we're using our "short circuit" technique and listing all of the agency spacecraft. You can also note that we are passing a function as a prop that you can probably guess will navigate to a spacecraft page.
...
7. Let's create a spacecraft page that loads the data.
   ...
   - within that file let's add the following code:
      ```jsx
      import Typography from '@mui/material/Typography';
      import Container from '@mui/material/Container';
      import Grid from '@mui/material/Grid';
